...
class GeometryGenerator {
    ...
    static Mesh Sphere(glm::vec3 pos, float radius, int tessellation) {
        ...
        for (unsigned int lat = 1; lat < tessellation; ++lat) {
            ...
            for (unsigned int lon = 0; lon <= tessellation; ++lon) {
                ...
                vertices.back().tangent = -glm::cross(vertices.back().normal,
    ...
    static Mesh Cube(glm::vec3 pos, glm::vec3 scale) {
        ...
        indices = {
            ...
            8,10,9,9,10,11,12,14,13,
            13,14,15,16,18,17,17,18,
            19,20,22,21,21,22,23,
        };
        ...
        for (auto& vertex : vertices) {
            auto vertPos = transform * glm::vec4(vertex.position, 1.0f);
            vertex.position = glm::vec3(vertPos.x, vertPos.y, vertPos.z);
